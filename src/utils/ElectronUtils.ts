...
export const setZoomFactor = (zoomFactor: number) => {
  electron.webFrame.setZoomFactor(zoomFactor);
};

export const adjustZoom = (params?: {
  ...
  const originalInnerHeight =
    typeof params.originalInnerHeight !== 'undefined' ? params.originalInnerHeight : 1080;
  const originalScaleFactor =
    typeof params.originalScaleFactor !== 'undefined' ? params.originalScaleFactor : 1.0;
  const originalDevicePixelRatio =
    typeof params.originalDevicePixelRatio !== 'undefined' ? params.originalDevicePixelRatio : 1.0;
...
export const disableManualZoom = () => {
  window.onkeydown = (e: KeyboardEvent) => {
    // e.metaKey = CMD key on macOS
    if ((e.ctrlKey || e.metaKey) && (e.code == 'Minus' || e.code == 'Equal')) {
      e.preventDefault();
    }
  };
};

export const forceDeviceScaleFactorToOneOnWin32 = () => {
...
export const getAppDataFolder = (companyName: string, appName: string) => {
...
export const readPackageJson = () => {
  if (isDevelopment) {
    ...
    return JSON.parse(fs.readFileSync(path.join(remote.app.getAppPath(), 'package.json'), 'utf-8'));
  }
};
